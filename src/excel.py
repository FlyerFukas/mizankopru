...
def main():
    ...
    grup_ici = oku(ARA_DIZIN / "grup_ici.csv")
    ...
    eslesmeyenler = pd.read_csv(ARA_DIZIN / "eslesmeyenler.csv",
                                encoding="utf-8-sig", dtype={"yerel_hesap_kod": str})
    ...
    ozet_satirlar = [
        ...
        ("Brüt marj", brut / hasilat, "yuzde"),
        ("Faaliyet marjı", faaliyet / hasilat, "yuzde"),
        ("Net marj", net / hasilat, "yuzde"),
    ...
    r = P.tablo_basligi(s, r, ["Şirket", "Ünvan", "Ülke", "Para birimi",
                               "Hesap planı"])
    for kod, sir in y.sirketler.items():
    ...
    s.write(r, 1, ", ".join(f"{k} %{v.sahiplik_orani*100:.0f}"
                            for k, v in y.sirketler.items()), B["metin"])
    ...
    for ad, deger, kalin in [("Hasılat", hasilat, False),
        ...
        s.write_number(r, 3, float(deger / hasilat),
    ...
    for ad in ["Dönen varlıklar", "Duran varlıklar", "Kısa vadeli yükümlülükler",
        ...
        r += 1
    r += 1
    s.write(r, 0, "Dönem net kârı (gelir tablosu kapatılmadı)", B["metin"])
    s.write_number(r, 1, float(net), B["sayi"])
    ...
    s, r = P.sayfa("Sapma köprüsü", [30, 18, 14, 16, 16, 16, 16, 16],
                   ...
                   "EUR/TRY 38,00'de sabitlenmişti, gerçekleşen yıl sonu 50,60.")
    s.write(r, 0, "GRUP TOPLAMI", B["bolum"]); r += 1
    r = P.tablo_basligi(s, r, ["Kalem", "EUR", "Bütçeye oran"])
    ...
    r = P.tablo_basligi(s, r, ["Şirket", "Bütçe EUR", "PB", "Fiyat", "Karışım",
                               "Hacim", "Kur", "Fiili EUR"])
    ...
    r = P.tablo_basligi(s, r, ["Satıcı", "Alıcı", "Dönem", "Tür",
                               "Alacak (EUR)", "Borç (EUR)", "Fark", "Durum"])
    kayit = {}
    ...
    r = P.tablo_basligi(s, r, ["Dönem", "Şirket", "Yerel denksizlik",
                               "Veri hatası EUR", "Saf çevrim farkı EUR", ""])
